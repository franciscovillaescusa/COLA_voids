...
#QUINTIC_KERNEL             # Implementation of the Morris 1996 quintic spline kernel
...
#WRITE_SUB_COLLECTIVELY             # Save subfind results collectively in as many files as per snapshot
...
#INTERP_OBSERVER_FRAME=144          # Interolates between the original tables to allow to produce outputs at any time
...
#SUBFIND_GROUP_SUBCOMM               #allows collective version to perform on subset of MPI tasks
...
#MODGRAV_GR_LIMIT              # runs the MG-Gadget package in the GR limit 
#MODGRAV_SEPARATE_ACCEL        # enables separate handling of modified gravity acceleration
#MODGRAV_PM_TIMESTEP           # calculate MG-acceleration only at PM-timestep (requires MODGRAV_SEPARATE_ACCEL)
...
#GRADPOTENTIAL           # computes and outputs the gradient of the gravitational potential
...
#-------------------------------------- GM BH repositioning and energy deposition scheme
#GM_BH_EVAPORATE_COLD_CLOUDS     # Put a gas particle out of star formation if BH energy is enough to destroy the cold phase
#GM_BH_FORCE_THEOR_MASS          # BH mass always equal to theoretical one (to use with NO swallow)
#GM_BH_MOVE_ON_GRAV_SOFT=1.0     # BHs go to minimum potential position only within this scale
#GM_MAX_FRAC_HSML_REPOSITION     # As above but referred to a fraction of the BHs HSML (mutually exclusive)
#GM_BHMERGERS_IN_MINPOT          # BHs merger goes in the minimum potential position among all of the merged BHs
#GM_BHMERGERS_GRAVSOFT_FACT=1.0  # BHs mergers only happen within this scale
...
##NO_ISEND_IRECV_IN_PM               # no ISEND IRECV in ImportFromL3/pm_periodic_l3.c
##FIX_PATHSCALE_MPI_STATUS_IGNORE_BUG
#BCAST_EWALD_TABLE                 # Ewald correction tables are read by root and broadcast
#NOTREEINRESTARTFILE               # tree is not saved in the restart file, a new domain decomposition is done when restarting
...
#LOCALIZED_SOFTENINGS=63   # can be used to equalize softenings at the node-level to more efficiently allow node softening to apply (the affected types are given through a bit mask)
...
#CONDUCTION_INCLUDEMAGNETIC                   # Main switch for anisotropic (including magnetic field) thermal conduction
#CONDUCTION_INCLUDEMAGNETIC_EASY              # Simplified version, not recommended
#CONDUCTION_INCLUDEMAGNETIC_ISOTROPISED       # Isotropic version for cases with very strong anisotropies
#CONDUCTION_INCLUDEMAGNETIC_PERPENDICULAR     # Correct treatment of conduction in presence of very weak magnetic fields
...
#AB_ART_VISC=4.0            # Variant of C&D 2010 viscosity
...
#TIME_DEP_ART_COND          # Time dependent artificial conductivity scheme
#AB_COND_GRAVITY=5.0        # Limits artificial conduction in presence of gravity
...
#--------------------------------------- Matteo Nori (April 2017): Axion w/ SPH Madelung formalism

#AXION_DM

#SPH_ONLY_APPROACHING
#GAMMA_AXION=2
#TYPE_AXION=1
...
# AXION_DM:
# type TYPE_AXION (ifndef -> 1) particles are treated as Ultra Light Axions
# which dynamic is solved using an SPH routine (Pressure (if GAMMA_AXION =! 0) + Quantum Pressure)
# coming from Schrödinger eq with Madelung form field. (see Mocz & Succi, Phys.Rev. E91 (2015) no.5, 053304 )

# ONLY_APPROACHING:
# disable the Quantum Pressure SPH acceleration contibution for receding particle couples.

# GAMMA_AXION :
# sets the gamma exponent of the Equation of State for Axions.
# Default is 0 -> no self-interaction (aka pressure)
...
#EOS_NSPECIES=3
#WAKEUP=4.0           # allows 2 timestep bins within kernel
#WAKEUP_NOCORRECTION  # Wakeup particles, but do not correct the DKD operator
#RELAXOBJECT
